...
class PilotNetInferenceRuntime:
    ...
    def frame_stack(self) -> int:
        return int(self.model_config.get("frame_stack", max(1, int(self.model_config.get("image_channels", 3)) // 3)))
    ...
    def predict_steer(
        ...
        *,
        ...
        if self.model_config.get("command_conditioning", "none") in {"embedding", "branch"}:
            command_index = torch.tensor([command_to_index(command)], dtype=torch.long, device=self.device)
        ...
        if self.requires_route_point():
            ...
            route_point_tensor = torch.tensor([route_point], dtype=torch.float32, device=self.device)
