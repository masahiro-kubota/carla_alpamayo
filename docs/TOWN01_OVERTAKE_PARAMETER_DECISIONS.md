...
現時点で、Town01 向けの初期値として採用する値は次です。
...
| `overtake_min_front_gap_m` | `20.0` | Town01 の低速環境では十分保守的で、短い直線区間でも過剰 rejection になりにくい |
...
| `overtake_lane_marking_allow_rule` | `left marking = Broken Yellow only` | Town01 safe route 2 本はどちらも `Broken Yellow` だった |
