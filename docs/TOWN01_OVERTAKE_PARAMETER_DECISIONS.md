...
| `overtake_lane_marking_blocklist` | `all left markings except Broken Yellow` | Town01 safe route 2 本はどちらも `Broken Yellow` だった |
...
## 10. `overtake_lane_marking_blocklist`

### 結論

- Town01 の P0 overtake は `left lane marking = Broken Yellow` のときだけ許可する
- requirements との対応としては、
  - `overtake_lane_marking_blocklist = all left markings except Broken Yellow`
  と表現する
...
- `allow rule` と `blocklist` のどちらで実装してもよいが、decision doc では requirements に合わせて `blocklist` で表現する
...
- `target_lane_id`
- `event_overtake_attempt`
- `event_overtake_success`
- `event_overtake_abort`
